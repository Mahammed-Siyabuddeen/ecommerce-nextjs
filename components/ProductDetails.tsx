        ...
        <div className="flex gap-2">
          {
            productdetials.sizes?.map((size, index) => (
              ...
              <div key={index} onClick={() => setCurrentSize(size)} className={`p-2  px-4 border cursor-pointer border-2 rounded ${size === currentsize ? 'border-blue-600' : ''}`}>{size}</div>
            ))
          }
        </div>
        <div className="flex gap-4">
          <button onClick={handleBuyNow} className="px-5 py-3 max-w-fit font-medium gap-2 text-center inline-flex items-center text-gray-950 bg-yellow-400 rounded-lg hover:bg-slate-50  ">
            <ShopIcon />  Buy Now
          </button>
