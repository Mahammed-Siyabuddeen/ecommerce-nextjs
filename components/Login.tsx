...
import { adminLoginApi } from '@/Services/adminLogin.service';
...
const Login = ({ admin }: { admin: boolean }) => {
    ...
    return (
        <form onSubmit={(e: FormEvent) => handleClick(e)} className="container ml-auto mr-auto w-5/6 md:w-2/6 my-12 flex flex-col gap-4">
            ...
            {!admin && <Link href={'/forget-password'} className="font-medium text-end  text-yellow-400 cursor-pointer">forget Password?</Link>}
